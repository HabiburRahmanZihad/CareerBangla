...
import RecruiterJobsSkeleton from "./RecruiterJobsSkeleton";
...
const RecruiterJobsByStatusContent = ({ title, description, status, emptyMessage }: RecruiterJobsByStatusContentProps) => {
    ...
    if (isLoading) {
        return <RecruiterJobsSkeleton bgClass={currentConfig.bg} />;
    }
                ...
                <div className={layoutMode === "grid" ? "grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6" : "flex flex-col gap-5"}>
                    {jobs.map((job) => (
                        <RecruiterJobCard
                            ...
                            statusBadgeClass={currentConfig.badge}
                            statusText={status}
                            onDelete={(id: string) => {
                            ...
                            isDisabled={deleting || isFetching}
                        />
                    ))}
                </div>
            )}
